...
function init_include()
    ...
    include('Wiz-SelfCommands')

    -- Include general user globals, such as custom binds or gear tables.
    -- Load Wiz-Globals first, followed by User-Globals, followed by <character>-Globals.
    ...
    -- Global default binds (either from Wiz-Globals or user-globals)
...
-- Global  - user_xxx() functions get called first.  Define in Wiz-Globals or User-Globals.
...
function get_melee_set()
    ...
    if meleeSet[state.CombatMode.current] then
        meleeSet = meleeSet[state.CombatMode.current]
        mote_vars.set_breadcrumbs:append(state.CombatMode.current)
    end
